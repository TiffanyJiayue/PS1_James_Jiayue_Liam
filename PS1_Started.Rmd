...
**Group Member 2: James**

**Group Member 3: Jiayue**
...
**Code:**
```{r,sim2a,echo= TRUE}
sim1<-lm(data=out, Y~V1+V2)
```

```{r, warning=FALSE,sim2b,echo= TRUE, results = "asis"}
stargazer(sim1, type="latex", header=FALSE, 
          title="Omitted Variable Bias Simulation", omit.stat=c("f", "ser"))
```

**Answer:**
Based on Table1, our estimated $\beta_1$  is 2.991, which is similar to our $\beta_1$  assigned value '3'. And estimated $\beta_2$ is 6.044, which is also similar to our $\beta_2$ assigned value '6'. Therefore, our estimates of $\beta_1$ and $\beta_2$ are not biased.
\clearpage
...
```{r, warning=FALSE,sim2d,echo= TRUE, results = "asis"}
stargazer(sim2, type="latex", header=FALSE, 
          title="Omitted Variable Bias Simulation", omit.stat=c("f", "ser"))
```

**Answer:**
Based on Table2, our estimated $\beta_1$  is 7.826, which is not similar to our $\beta_1$  assigned value '3'.  Therefore, our estimates of $\beta_1$ is biased.
...
```{r,sim3a,echo= TRUE}
model_adj<-lm(Y_adj~V1, data=out)
```

```{r, warning=FALSE,sim3b,echo= TRUE, results = "asis"}
stargazer(model_adj, type="latex", header=FALSE, 
          title="Omitted Variable Bias Simulation", omit.stat=c("f", "ser"))
```

**Answer:**
Based on Table3, our estimated $\beta_1$  is 3.026, which is similar to our $\beta_1$  assigned value '3'. This indicates that our estimates of $\beta_1$ is not biased, because when we generate $Y_{adj}=Y-\beta_2*V_2$, the $Y_{adj}$ already includes the effect of $V_2$, therefore, we can find the unbiased estimate of $\beta_1$, which is the effect of $V_1$.
...
```{r key 4}
plotted <- ggplot(out, aes(V1)) + 
    geom_point(aes(y = Y, col = "Y")) + 
    geom_point(aes(y = Y_adj, col = "adjY")) +
    geom_smooth(method='lm', aes(y = Y, col = "Y")) +
    geom_smooth(method='lm', aes(y = Y_adj, col = "adjY")) +
...
**Code:**
```{r}
cps_data <- read.csv("cps_clean.csv")
model_income_education <- lm(inctot ~ edu, data = cps_data)
summary(model_income_education)

```

**Answer:**

one additional year of education will add $8,114.70 to ones salary. 
...
**Code:**
```{r}
model_income_controls <- lm(inctot ~ edu + female + age , data = cps_data)
summary(model_income_controls)
```

**Answer:**

The increases in salary per year is 8,296, holding all else constant,  which is similar to the coefficient without the control variable.
